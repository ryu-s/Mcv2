...
namespace Mcv.MainViewPlugin
{
    ...
    class Adapter : IMainViewHostAdapter, IPluginMainHost
    {
        ...
        public async Task LoadUserStoreAsync()
        {
        ...
        public async Task SaveUserStoreAsync()
        {
            var res = await _host.RequestMessageAsync(new GetPluginSettingsDirPath(_userDataDbPath)) as ReplyPluginSettingsDirPath;
            if (res is null)
            {
        ...
        public void OnConnectionStatusChanged(IConnectionStatusDiff connStDiff)
        {
            ConnectionStatusChanged?.Invoke(this, new ConnectionStatusChangedEventArgs(connStDiff));
        }

        public void OnMessageReceived(NotifyMessageReceived messageReceived, MyUser? user)
        {
            MessageReceived?.Invoke(this, new MessageReceivedEventArgs(messageReceived, user));
        }

        public void OnPluginAdded(IPluginInfo pluginInfo)
        {
            PluginAdded?.Invoke(this, new PluginAddedEventArgs(pluginInfo.Id, pluginInfo.Name));
        }
        public void OnPluginAdded(PluginId pluginId, string pluginName)
        {
            PluginAdded?.Invoke(this, new PluginAddedEventArgs(pluginId, pluginName));
        }

        public void OnMetadataUpdated(NotifyMetadataUpdated metadataUpdated)
        {
        ...
        public void OnSiteAdded(PluginId siteId, string siteDisplayName)
        {
        ...
        public void OnConnectionAdded(IConnectionStatus connSt)
        {
        ...
        public void RequestChangeConnectionStatus(ConnectionStatusDiff connectionStatusDiff)
        {
        ...
        public void RequestShowSettingsPanel(PluginId pluginId)
        {
            _host.SetMessageAsync(new RequestShowSettingsPanel(pluginId));
        }
        public async Task<string> GetSitePluginDisplayName(PluginId pluginId)
        {
        ...
        public async void AfterInputChanged(ConnectionId connId, string input)
        {
            var sites = _sitePlugins.ToArray();
            foreach (var (site, _) in sites)
            {
        ...
        public async Task SetConnectSite(PluginId selectedSite, ConnectionId connId, string input, BrowserProfileId browserProfileId)
        {
        ...
        public async Task<(bool updateExists, string url, string current, string latest)> CheckIfUpdateExistsAsync()
        {
        ...
        public void SuggestToUpdate(string url, string current, string latest)
        {
            SuggestToUpdateEvent?.Invoke(this, new SuggestToUpdateEventArgs(url, current, latest));
        }

        public void RequestUpdate(string latest, string url)
        {
