...
namespace BigoSitePluginTests
{
    ...
    public class Class1
    {
        ...
        public void Parse()
        {
            ...
            Assert.That(internalMessage?.Message, Is.EqualTo("@‭💫‬THANH LIEM‭💫‬ おめでとうございます🤗おつです🙇‍♂️"));
            Assert.That(internalMessage?.Name, Is.EqualTo("ゆき\U0001f994🌸とし🐻❤️"));
        }
        ...
        public void ParseNormalGiftTextTest()
        {
            ...
            Assert.That(internalMessage, Is.Not.Null);
            Assert.That(internalMessage?.A, Is.EqualTo("0"));
            Assert.That(internalMessage?.B, Is.EqualTo("0"));
            Assert.That(internalMessage?.C, Is.EqualTo("1"));
            Assert.That(internalMessage?.K, Is.EqualTo(""));
            Assert.That(internalMessage?.M, Is.EqualTo("2662"));
            Assert.That(internalMessage?.N, Is.EqualTo("ポッカリ"));
            Assert.That(internalMessage?.T, Is.EqualTo(""));
            Assert.That(internalMessage?.U, Is.EqualTo(""));
        }
        ...
        public void ParseLightMyHeartTextTest()
        {
            ...
            Assert.That(internalMessage, Is.Not.Null);
            Assert.That(internalMessage?.ItemId, Is.EqualTo("495"));
            Assert.That(internalMessage?.Timestamp, Is.EqualTo(1609187381));
            Assert.That(internalMessage?.UserId, Is.EqualTo("491234139"));
            Assert.That(internalMessage?.Username, Is.EqualTo("Gracee"));
        }
    }
}
