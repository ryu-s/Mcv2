...
namespace Mcv.Core.V1
{
    class SQLiteUserStore : IUserStore
    {
        ...
        public IEnumerable<McvUser> GetAllUsers()
        {
            return _cacheDict.Values;
        }
        public McvUser GetUser(string userId)
        {
            ...
            userInfo = new McvUser(userId);
            AddUser(userInfo);
            return userInfo;
        }
        private void AddUser(McvUser userInfo)
        {
        ...
        private List<McvUser> LoadAllUserInfo()
        {
            CreateDB(_dbPath);

            var query = $"SELECT {col1Name},{col2Name},{col3Name} FROM {tableName}";
            var list = new List<McvUser>();
            try
            {
        ...
        public static string ToJson(McvUser user)
        {
            var json = "{"
                + $"\"userid\":\"{user.UserId}\","
        ...
        private int UpdateUser(SQLiteCommand cmd, McvUser user)
        {
            var json = ToJson(user);
        ...
        private void InsertUser(SQLiteCommand cmd, McvUser user)
        {
            var json = ToJson(user);
