...
namespace Mcv.YouTubeLiveSitePlugin
{
    ...
    class CommentProviderNext : CommentProviderBase, IYouTubeCommentProvider
    {
        ...
        private static async Task<LiveChatHtml> GetLiveChat(string vid, CookieContainer cc)
        {
            ...
            return new LiveChatHtml(await res.Content.ReadAsStringAsync());
        }
        ...
        private async Task ConnectInternalAsync(IInput input, List<Cookie> cookies)
        {
...
reload:
        ...
        private YouTubeLiveMessageContext CreateMessageContext2(TextMessage text, bool isInitialComment)
        {
            ...
            IEnumerable<Mcv.PluginV2.IMessagePart> commentItems;
            IEnumerable<Mcv.PluginV2.IMessagePart> nameItems;
            ...
            {
                var messageText = MessagePartsTools.ToText(commentItems);
        ...
        private YouTubeLiveMessageContext CreateMessageContext2(SponsorshipsGiftPurchaseAnnouncement text, bool isInitialComment)
        {
            ...
            if (_siteOptions.IsAutoSetNickname)
            {
                var messageText = MessagePartsTools.ToText(message.MessageItems);
                var nick = Utils.ExtractNickname(messageText);
                if (!string.IsNullOrEmpty(nick))
                {
                    newNickname = nick;
        ...
        private YouTubeLiveMessageContext CreateMessageContext2(MemberShip text, bool isInitialComment)
        {
            ...
            if (_siteOptions.IsAutoSetNickname)
            {
                var messageText = MessagePartsTools.ToText(message.CommentItems);
                var nick = Utils.ExtractNickname(messageText);
                if (!string.IsNullOrEmpty(nick))
                {
                    newNickname = nick;
        ...
        public override async Task ConnectAsync(string input, List<Cookie> cookies)
        {
            ...
            try
            {
                ...
                await ConnectInternalAsync(p, cookies);
            }
            finally
            {
                AfterDisconnected();
        ...
        public CommentProviderNext(IYouTubeLiveServer server, YouTubeLiveSiteOptions siteOptions, ILogger logger)
            : base(logger)
        {
            _siteOptions = siteOptions;
            _logger = logger;
