...
namespace TwicasSitePluginTests
{
    ...
    class StreamCheckerParseTests
    {
        ...
        public void Test()
        {
            ...
            Assert.That(sc.Telop, Is.EqualTo("有名キャス歌い手集団の裏"));
            return;
        }
        [Test]
        public void Test2()
        {
            ...
            var sc = TwicasSitePlugin.LowObject.StreamChecker2.ParseStreamChcker(data);
            Assert.That(sc.Telop, Is.EqualTo("僕よりイケボな人凸待ち"));
            return;
        }
        ...
        public void KiitosTest()
        {
            ...
            Assert.That(item.Effect, Is.EqualTo("stampitem(\"https://twitcasting.tv/img/stamp/stamp_funding_100.png\",1,3000)"));
            Assert.That(item.Id, Is.EqualTo("3639"));
            Assert.That(item.ItemImage, Is.EqualTo("https://twitcasting.tv/img/item_funding_stamp.png"));
            Assert.That(item.Message, Is.EqualTo("達成: 100% : 💛リスナーさんのコメントで三角コーナー飯作る💛"));
            Assert.That(item.SenderImage, Is.EqualTo("https://twitcasting.tv/img/item_funding_user.png"));
            Assert.That(item.SenderScreenName, Is.EqualTo("kiitos_cas"));
        ...
        public void ContinueCoinTest()
        {
            ...
            Assert.That(item.Effect, Is.EqualTo(""));
            Assert.That(item.Id, Is.EqualTo("3703"));
            Assert.That(item.ItemImage, Is.EqualTo("https://s01.twitcasting.tv/img/item_coin.png"));
            Assert.That(item.Message, Is.EqualTo("オモロイ人だね"));
            Assert.That(item.SenderImage, Is.EqualTo("https://imagegw02.twitcasting.tv/image3/img-twitcasting.s3-us-west-1.amazonaws.com/9a/1f/5be434d22723a_64.jpg"));
            Assert.That(item.SenderScreenName, Is.EqualTo("c:haru5576"));
            Assert.That(item.t13, Is.EqualTo("コンティニューコイン"));
            var screenName = item.SenderName;
            var username = item.SenderScreenName;
        ...
        public void FrameTest()
        {
            ...
            Assert.That(item.Effect, Is.EqualTo("frameitem(\"https://twitcasting.tv/img/anim/anim_frame_burger_v2_5.png\")"));
            Assert.That(item.Id, Is.EqualTo("3707"));
            Assert.That(item.ItemImage, Is.EqualTo("https://s01.twitcasting.tv/img/item_frame_child.png"));
            Assert.That(string.IsNullOrEmpty(item.Message), Is.True);
            Assert.That(item.SenderImage, Is.EqualTo("https://imagegw02.twitcasting.tv/image3s/pbs.twimg.com/profile_images/1082451300220989440/46bLVM4F_normal.jpg"));
            Assert.That(item.SenderScreenName, Is.EqualTo("12_Dahlia_09"));
            Assert.That(item.t13, Is.EqualTo("ペッパー・デコフレーム"));
            var screenName = item.SenderName;
            var username = item.SenderScreenName;
            var message = item.Message;
        }
